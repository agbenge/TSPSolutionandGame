...
MeasureSpec.UNSPECIFIED : It can take as much space the view wants
    ...
    protected void drawWithZoom(Canvas canvas) {
        ...
        Path gc = new Path();
        gc.reset();
        float x = (float) (((pointXY.get(0).x + invertNegavative + padding) * zoom) - padding * zoom) + padding;
        float y = (float) (((pointXY.get(0).y + invertNegavative + padding) * zoom) - padding * zoom) + padding;
        gc.moveTo(x, y);
        float i_x=x,i_y=y;
        for (Integer i : path) {
            x = (float) (((pointXY.get(i).x + invertNegavative + padding) * zoom) - padding * zoom) + padding;
            y = (float) ((pointXY.get(i).y + invertNegavative + padding) * zoom - padding * zoom) + padding;
            gc.lineTo(x, y);
